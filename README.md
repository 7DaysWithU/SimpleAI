...
存放**自定义数据集(`CustomDataset` 子类)**。每个数据集必须继承`CustomDataset`基类，该基类同时继承`ABC`与PyTorch的`Dataset`，要求子类实现`__getitem__()`方法以支持索引访问，并可选择性重写`__len__()`；此外，所有子类均可直接使用基类提供的静态方法`train_valid_test_split()`实现标准化的数据划分。
